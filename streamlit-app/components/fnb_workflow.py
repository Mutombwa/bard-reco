        ...
        try:
            ...
            if 'matched' in results and not results['matched'].empty:
                ...
                if not perfect_matches.empty:
                    ...
                    # Add column headers
                    csv_rows.append(perfect_matches.columns.tolist())
                    
                    # Add data rows
                    for _, row in perfect_matches.iterrows():
                        csv_rows.append(row.tolist())
    ...
    def display_fnb_category(self, results, category):
        ...
        if category == 'matched':
            ...
            if split_matches:
                ...
                for i, split in enumerate(split_matches):
                    with st.expander(f"Split #{i+1} - {split.get('split_type', 'Unknown')}", expanded=True):
                        if split.get('split_type') == 'many_to_one':
                            st.write(f"**Statement Index:** {split['statement_idx']}")
                            st.write(f"**Ledger Indices:** {split['ledger_indices']}")
                        else:
                            st.write(f"**Ledger Index:** {split['ledger_idx']}")
                            st.write(f"**Statement Indices:** {split['statement_indices']}")
                return
            else:
                st.info("No split transactions found")
                return
