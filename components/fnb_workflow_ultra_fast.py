...
class UltraFastFNBReconciler:
    ...
    def _find_split_transactions(self, ledger: pd.DataFrame, statement: pd.DataFrame,
        ...
        # Relaxed similarity threshold specifically for split detection
        # Allows matches like 'THULANI' -> 'THULANI PAYMENT' to be considered
        split_similarity = settings.get('split_similarity_ref', max(50, similarity_threshold - 20))
        ...
        many_to_one = self._find_many_ledger_to_one_statement(
            ...
            match_dates, match_refs, split_similarity, fuzzy_enabled, date_tolerance
        )
    ...
    def _find_many_ledger_to_one_statement(self, ledger: pd.DataFrame, statement: pd.DataFrame,
        ...
        for s_idx, s_row in statement.iterrows():
            ...
            # Fallback: include small-amount ledger entries (bucket 0) which
            # may be part of split combinations (e.g. large amount + small fee)
            if len(potential_matches) < 2 and 0 in ledger_by_amount_range:
                for l_idx, l_amt in ledger_by_amount_range.get(0, []):
                    if l_idx not in candidates:
                        continue
                    # avoid duplicates
                    if any(p[0] == l_idx for p in potential_matches):
                        continue
            ...
            if len(potential_matches) > 20:
                ...
                potential_matches.sort(key=lambda x: (abs(abs(x[2] or 0) - abs(s_amt or 0)), -x[3]))
    ...
    def _find_one_ledger_to_many_statement(self, ledger: pd.DataFrame, statement: pd.DataFrame,
                                          ...
                                          similarity_threshold: int, fuzzy_enabled: bool,
                                          date_tolerance: bool) -> List[Dict]:
        """Find cases where one ledger entry matches multiple statements - USING REFERENCE GROUPING"""
        matches = []
        MAX_SPLITS = 50  # Limit to prevent excessive processing
        ...
        for l_idx, l_row in ledger.iterrows():
            ...
            if pd.isna(l_amt) or pd.isna(l_ref):
                continue
            ...
            for s_idx in candidates:
                ...
                if pd.isna(s_ref) or s_ref == '':
                    continue
                
                # Group by exact reference text (already normalized to uppercase)
                if s_ref not in reference_groups:
                    reference_groups[s_ref] = []
                reference_groups[s_ref].append(s_idx)

            # STEP 2: Find the best matching reference group
            best_ref_group = None
            best_ref_score = 0
            
            if match_refs and fuzzy_enabled:
                # Find reference group with highest similarity to ledger
                for s_ref, group_indices in reference_groups.items():
                    if len(group_indices) >= 2:  # Need at least 2 items for a split
                        cache_key = (l_ref, s_ref)
                        if cache_key in self.fuzzy_cache:
                            score = self.fuzzy_cache[cache_key]
                        else:
                            score = fuzz.ratio(l_ref, s_ref)
                            self.fuzzy_cache[cache_key] = score
                        
                        if score >= similarity_threshold and score > best_ref_score:
            ...
            if best_ref_group and len(best_ref_group) >= 2:
                # Build potential matches from the best reference group
                potential_matches = []
                for s_idx in best_ref_group:
                    s_row = statement.iloc[s_idx]
                    s_amt = s_row.get('_amount')
                    if pd.notna(s_amt):
                        potential_matches.append((s_idx, s_row, s_amt, best_ref_score))
                
                if len(potential_matches) >= 2:
                    # Find combination using DP with 2% tolerance (allows small rounding differences)
                    combination = self._find_split_combination_dp(potential_matches, l_amt, tolerance=0.02)
                    
                    if combination:
                        # VALIDATION: Calculate total amount from statement items
                        stmt_total = sum(abs(c[2]) for c in combination)
                        ledger_amount = abs(l_amt)
                        
                        # Allow 2% tolerance for foreign credits/debits
                        amount_difference = abs(stmt_total - ledger_amount)
                        percentage_diff = (amount_difference / ledger_amount * 100) if ledger_amount > 0 else 100
                        amounts_match = (percentage_diff <= 2.0)  # Allow 2% difference
    ...
    def _find_split_combination_dp(self, candidates: List[Tuple], target: float,
                                   tolerance: float = 0.05) -> Optional[List]:
        """
        ...
        Default tolerance is 5% to handle foreign credits/debits
        """
